...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	async function showSearchableSelector<T>(
	...
	async function showBranchSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const result = await showSearchableSelector<string>(
			ctx, "Select base branch", items, "No matching branches",
			(item) => item.value,
		);
	...
	async function showCommitSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const result = await showSearchableSelector<{ sha: string; title: string }>(
			ctx, "Select commit to review", items, "No matching commits",
			(item) => {
				const commit = commits.find((c) => c.sha === item.value);
				return commit ? { sha: commit.sha, title: commit.title } : null;
			},
		);
